...
from modules import MedRxivPaperClient


QUERY = "machine learning"
TOTAL_RESULTS = 5
DOWNLOAD_RESULTS = 2
OUTPUT_DIR = Path("downloads/MedRxivPaperClient")


async def main() -> None:
    async with MedRxivPaperClient(
        concurrency=3,
        show_progress=True,
        progress_mode="auto",
        verbose=True,
        ...
        print(f"Found {len(paper_infos)} medRxiv papers")
        ...
        if not selected_papers:
            print("No medRxiv papers to download")
            return
