'''
Function:
    Test search and download for BioRxivPaperClient
Author:
    Zhenchao Jin
...
from paperdl import BioRxivPaperClient


QUERY = "deep learning"
TOTAL_RESULTS = 5
DOWNLOAD_RESULTS = 2
OUTPUT_DIR = Path("downloads/BioRxivPaperClient")


async def main() -> None:
    async with BioRxivPaperClient(
        concurrency=3,
        show_progress=True,
        progress_mode="auto",
        verbose=True,
        ...
        print(f"Found {len(paper_infos)} bioRxiv papers")
        ...
        if not selected_papers:
            print("No bioRxiv papers to download")
            return
