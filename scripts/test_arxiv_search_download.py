'''
Function:
    Test search and download for ArxivPaperClient
Author:
    Zhenchao Jin
...
TOTAL_RESULTS = 5
DOWNLOAD_RESULTS = 2
OUTPUT_DIR = Path("downloads/ArxivPaperClient")


async def main() -> None:
    async with ArxivPaperClient(
        concurrency=3,
        show_progress=True,
        progress_mode="auto",
        verbose=True,
        ...
        print(f"Found {len(paper_infos)} arXiv papers")
        ...
        if not selected_papers:
            print("No arXiv papers to download")
            return
