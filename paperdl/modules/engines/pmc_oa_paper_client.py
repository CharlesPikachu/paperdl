...
class PMCOAPaperClient(BasePaperClient):
    ...
    def baseparams(self) -> dict[str, str]:
        params = {"tool": self.tool, **({"email": self.email} if self.email else {}), **({"api_key": self.api_key} if self.api_key else {})}
        return params
    '''queryids'''
    async def queryids(self, query: Optional[str], *, retstart: int = 0, retmax: int = 100, sort: str = "relevance") -> tuple[list[str], int]:
        ...
        payload: dict = await self.requestjson(self.ESEARCH_URL, params={**self.baseparams(), "db": "pmc", "term": term, "retmode": "json", "retstart": int(retstart), "retmax": int(retmax), "sort": sort})
        return (result := payload.get("esearchresult", {}) or {}).get("idlist", []) or [], int(result.get("count") or 0)
    '''fetchsummaries'''
    async def fetchsummaries(self, ids: list[str]) -> list[dict[str, Any]]:
        if not ids or not isinstance(ids, list): return []
        payload: dict = await self.requestjson(self.ESUMMARY_URL, params={**self.baseparams(), "db": "pmc", "id": ",".join(ids), "retmode": "json"})
    ...
    async def search(self, query: Optional[str] = None, *, total_results: int = 100, page_size: int = 50, sort: str = "relevance", require_pdf: bool = True, deduplicate: bool = True) -> list["PaperInfo"]:
        ...
        page_size, retstart, total_count = max(1, min(int(page_size), 200)), 0, None; paper_infos: list[PaperInfo] = []
        ...
        while len(paper_infos) < total_results:
            ...
            if not ids or not isinstance(ids, list): break
            ...
            if require_pdf: page_infos = [p for p in await asyncio.gather(*[self.attachoadownloadurl(p) for p in page_infos]) if p and p.download_url]
            paper_infos.extend(page_infos); retstart += page_size
            if (total_count is not None and retstart >= total_count) or (retstart >= page_size * 50 and len(paper_infos) == 0): break
        if deduplicate: paper_infos = list({p.identity_key: p for p in paper_infos}.values())
        self.updatetask(task_id, completed=1, description=f"Finished PMC OA search: {len((paper_infos := paper_infos[:int(total_results)]))} papers found")
        if self.progress_mode != "detailed": self.removetask(task_id)
        self.log(f"Finished PMC OA search. Found {len(paper_infos)} papers.")
        return paper_infos
    ''''pmcdownloadurlcandidates'''
    def pmcdownloadurlcandidates(self, url: str) -> list[str]:
        urls = [url]
        if (url.startswith("https://ftp.ncbi.nlm.nih.gov/pub/pmc/") and "/pub/pmc/deprecated/" not in url):
            urls.append(url.replace("https://ftp.ncbi.nlm.nih.gov/pub/pmc/", "https://ftp.ncbi.nlm.nih.gov/pub/pmc/deprecated/", 1,))
    ...
    async def downloaditem(self, paper_info: "PaperInfo", output_dir: str | Path = "paperdl_outputs", *, overwrite: bool = False, show_detail: bool = True) -> Path:
        if not paper_info.download_url: paper_info = await self.attachoadownloadurl(paper_info)
        if not (url := paper_info.download_url): raise PaperDownloadError(f"No PMC OA PDF URL available for: {paper_info.title}")
        path, last_error = Path(output_dir) / paper_info.filename(suffix=".pdf"), None
        short_title = paper_info.title[:67] + "..." if paper_info.title and len(paper_info.title) > 70 else paper_info.title
        for candidate_url in self.pmcdownloadurlcandidates(url):
            try: return await self.downloadvalidatedpdf(candidate_url, path, overwrite=overwrite, progress_description=f"Downloading: {short_title}", show_detail=show_detail, min_bytes=4 * 1024, min_pages=1)
            except Exception as err: last_error = err
        raise PaperDownloadError(f"Download failed for all PMC OA PDF URLs: {url}") from last_error
    '''attachoadownloadurl'''
    async def attachoadownloadurl(self, paper_info: "PaperInfo") -> "PaperInfo":
        if not (pmcid := paper_info.source_id or paper_info.extra.get("pmcid")): return paper_info
        try: pdf_url, license_name = await self.getoadownloadurl(pmcid)
        except Exception: return paper_info
    ...
    async def getoadownloadurl(self, pmcid: str) -> tuple[Optional[str], Optional[str]]:
        ...
        record = (root := ET.fromstring(xml_text)).find(".//record")
        ...
        for link in root.findall(".//link"):
            if link.attrib.get("format") == "pdf" and link.attrib.get("href"):
                if (href := link.attrib["href"]).startswith("ftp://ftp.ncbi.nlm.nih.gov/"): href = "https://ftp.ncbi.nlm.nih.gov/" + href.split("ftp://ftp.ncbi.nlm.nih.gov/", 1)[-1]
                if (href.startswith("https://ftp.ncbi.nlm.nih.gov/pub/pmc/") and "/pub/pmc/deprecated/" not in href): href = href.replace("https://ftp.ncbi.nlm.nih.gov/pub/pmc/", "https://ftp.ncbi.nlm.nih.gov/pub/pmc/deprecated/", 1)
    ...
    def summarytopaperinfo(self, item: dict[str, Any], *, query: Optional[str] = None, rank: Optional[int] = None) -> "PaperInfo":
        ...
        return PaperInfo(
            source=self.source, title=item.get("title") or "notitle", abstract=None, authors=authors, article_url=f"https://pmc.ncbi.nlm.nih.gov/articles/{pmcid}/" if pmcid else None, download_url=None, doi=self.extractdoi(item.get("elocationid")), venue=item.get("fulljournalname") or item.get("source"), 
            publisher="PubMed Central", published_at=item.get("pubdate") or item.get("epubdate"), source_id=pmcid, query=query, rank=rank, tags=["pmc", "open-access"], is_open_access=True, extra={"uid": item.get("uid"), "pmcid": pmcid, "pmid": item.get("pmid"), "raw": item},
        )
    '''extractdoi'''
    @staticmethod
