...
class PMLRPaperClient(BasePaperClient):
    ...
    async def listvolumes(self) -> list[int]:
        html = await self.requesttext(self.BASE_URL + "/", progress_description="Loading PMLR volumes")
        volume_ids = {int(m.group(1)) for m in re.finditer(r'href=["\'](?:https://proceedings\.mlr\.press)?/v(\d+)/?["\']', html)}
        return sorted(volume_ids, reverse=True)

    '''search'''
    async def search(self, query: Optional[str] = None, *, total_results: int = 100, volume_ids: Optional[Sequence[int]] = None, max_volumes: Optional[int] = 30, enrich_abstracts: bool = True, deduplicate: bool = True) -> list["PaperInfo"]:
        if not isinstance(total_results, (float, int)) or total_results <= 0: return []
        if volume_ids is None:
            volume_ids = await self.listvolumes()
            if max_volumes: volume_ids = list(volume_ids)[:max_volumes]
        paper_infos: list[PaperInfo] = []
        task_id = self.addtask(f"Searching PMLR: {(query or 'all')[:60]}", total=len(volume_ids), kind="generic")
        self.log(f"Start searching PMLR: query={query!r}, volumes={list(volume_ids)[:8]}...")
        ...
        self.updatetask(task_id, completed=len(volume_ids), description=f"Finished PMLR search: {len(paper_infos)} papers found")
    ...
    async def queryvolume(self, *, volume_id: int, query: Optional[str] = None, enrich_abstracts: bool = True) -> list["PaperInfo"]:
        url = f"{self.BASE_URL}/v{int(volume_id)}/"
        html = await self.requesttext(url)
        ...
        if enrich_abstracts:
            paper_infos = [p for p in await asyncio.gather(*[self.enrichpaperinfo(p) for p in paper_infos]) if p]
        return paper_infos
    ...
    '''blocktopaperinfo'''
    def blocktopaperinfo(self, block, *, volume_id: int, query: Optional[str] = None) -> "PaperInfo":
        title = self.getblocktext(block, ".title")
        details = self.getblocktext(block, ".details")
        article_url = None
        download_url = None
        for link in block.find_all("a", href=True):
            text, href = link.get_text(" ", strip=True).lower(), link["href"]
            href = self.absurl(href, volume_id=volume_id)
        ...
        if not title:
            text = block.get_text("\n", strip=True).split("\n")
            title = next((x for x in text if x and not x.lower() in {"abs", "download pdf"}), None)
        authors = self.extractauthorsfromdetails(details)
        year = self.extractyear(details)
    ...
    async def enrichpaperinfo(self, paper_info: "PaperInfo") -> "PaperInfo":
        ...
        try:
            html = await self.requesttext(paper_info.article_url)
            soup = BeautifulSoup(html, "lxml")
            title = self.getblocktext(soup, "h1") or paper_info.title
            abstract = self.extractabstract(soup)
            bibtex = soup.get_text("\n", strip=False)
            paper_info.title = title
            paper_info.abstract = abstract or paper_info.abstract
            ...
            authors = self.extractbibfield(bibtex, "author")
            if authors: paper_info.authors = [a.strip() for a in re.split(r"\s+and\s+", authors) if a.strip()]
            return paper_info
        except Exception:
            return paper_info
    ...
    async def downloaditem(self, paper_info: "PaperInfo", output_dir: str | Path = "paperdl_outputs", *, overwrite: bool = False, show_detail: bool = True) -> Path:
        if not paper_info.download_url:
            paper_info = await self.enrichpaperinfo(paper_info)
    ...
    '''getblocktext'''
    @staticmethod
    def getblocktext(block, selector: str) -> Optional[str]:
